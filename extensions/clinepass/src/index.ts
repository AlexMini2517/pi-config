...
import { resolveClineAuthCredentials } from "./workos.js";
...
export default async function (pi: ExtensionAPI) {
  ...
  // Attempt dynamic model discovery from the Cline API using either static API key
  // or active WorkOS OAuth token. Falls back to static MODELS on error.
  const activeToken = resolveApiKey() ?? resolveClineAuthCredentials()?.accessToken;
  const models = await resolveModels(activeToken, { apiBase });
  ...
  pi.on("before_provider_headers", (event) => {
    Object.assign(event.headers, buildClineCliHeaders());
  });
}
